...
> 角色：所有其它 6 个区块单元（`homepage-hero` / `homepage-feature-nav` / `homepage-city-grid` / `homepage-hot-posts` / `homepage-hot-spots` / `homepage-ai-launcher`）的**前置底座**
> 历史：早期 change `homepage-layout` 的命名与区域划分（platformNav/destinations/community）已废弃，本 spec 为当前权威定义。
...
- 任何 CSS 框架引入（Tailwind / shadcn 等属于独立 `frontend-styling-stack` change）
...
- [ ] `openspec/specs/http-server/spec.md` 中"通过前端 Next.js SSR 调后端访问"相关 Scenario 标注：首页数据获取由后续区块 change 负责，shell 阶段不依赖后端
