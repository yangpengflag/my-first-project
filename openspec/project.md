...
- **后端内容管理系统**：本期提供认证后的景点写 API（`POST` / `PUT /api/spots`，JWT 登录即可、creator 不持久化）与景点收藏 / 排行榜只读端点（见 `places` capability）；但**不搭建可视化 CMS 后台管理控制台**——审核流、富文本编辑 UI、角色权限矩阵、批量导入界面等仍在范围外。
